...
export default function UploadPage() {
  ...
  const employeeBands = t.options.employeeBands as Record<
    string,
    string
  >;
  const revenueBands = t.options.revenueBands as Record<
    string,
    string
  >;
  const assetLabels = t.options.highValueAssets as Record<
    string,
  ...
  function resolveBusinessType(): string {
    const activity = (
      editedFields.activity ??
      ocrResult?.fields.activity.value ??
      ''
    ).toLowerCase();
    const match = businessTypes.find((bt) => {
      ...
      return (
        activity.includes(title.split('/')[0].trim()) ||
        desc.split(',').some((w) => activity.includes(w.trim()))
      );
    });
  ...
  function handleGetQuotes() {
    ...
    const licenseNumber =
      editedFields.licenseNumber ??
      ocrResult.fields.licenseNumber.value;
  ...
  const stageIcon =
    progress.pct < 30 ? '📤' : progress.pct < 60 ? '🔍' : '✨';
  const canContinueFromReview =
    !!ocrResult && !hasInvalidOcrFields(ocrResult);
      ...
      <div className="max-w-3xl mx-auto px-4 w-full">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
          {step === 'upload'
            ? t.upload.title
            : step === 'review'
              ? t.upload.titleReview
              : t.upload.titleDetails}
        </h1>
      ...
      <div className="max-w-3xl mx-auto px-4 w-full flex flex-col gap-4">
        ...
        {step === 'upload' && (
          <>
            <div
              ...
              <div
                className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-colors duration-200 ${dragOver ? 'bg-primary/10' : 'bg-gray-100'}`}>
                <svg
                  width="32"
                  height="32"
                  viewBox="0 0 24 24"
            ...
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              ...
              <div className="flex flex-col gap-2 p-4">
                {fieldMeta.map(({key, label}) => {
                  ...
                  if (
                    !field ||
                    (!field.value && field.confidence === 'high')
                  )
                    return null;
                  return (
                    <EditableField
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                <svg
                  ...
                  className="text-primary">
                  <path
                    d="M2 8C2 4.686 4.686 2 8 2C10.21 2 12.117 3.273 13.064 5.143M14 8C14 11.314 11.314 14 8 14C5.79 14 3.883 12.727 2.936 10.857M2 8V4M2 8H6M14 8V12M14 8H10"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
              ...
              <button
                ...
                <svg
                  ...
                  className="text-primary">
                  <path
                    d="M11.333 2L14 4.667M1.333 14.667L2.067 11.72L10.067 3.72C10.333 3.453 10.733 3.453 11 3.72L12.333 5.053C12.6 5.32 12.6 5.72 12.333 5.987L4.333 13.987L1.333 14.667Z"
                    stroke="currentColor"
                    strokeWidth="1.2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
        ...
        {step === 'details' && (
          <>
            ...
            <div>
              <p className="text-sm font-medium text-gray-900 mb-1.5">
                {t.upload.estimatedRevenue}{' '}
                <span className="text-gray-400 font-normal">
                  {t.upload.next12Months}
                </span>
              </p>
              <p className="text-xs text-gray-400 mb-3">
                {t.upload.revenueHelper}
              </p>
              <div className="flex flex-col gap-2">
                      ...
                      <button
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            ...
                            {assetLabels[asset.id]?.label ??
                              asset.label}
                          </p>
                          <p className="text-xs text-gray-400">
                            {assetLabels[asset.id]?.description ??
                              asset.description}
                          </p>
                        </div>
                        <div
