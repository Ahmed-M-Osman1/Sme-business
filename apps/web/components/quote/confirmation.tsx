...
export function Confirmation() {
  ...
  const [policyNumber] = useState(
    () => {
      ...
      return `${policyPrefix}${Date.now().toString(36).toUpperCase()}`;
    },
  );
...
<head>
  <meta charset="utf-8">
  <title>${brand.displayName} Invoice - ${policyNumber}</title>
  <style>
...
<body>
  <div class="header">
    <div class="logo">${brand.displayName}</div>
  ...
  <div class="addresses">
    ...
    <div class="address-block">
      <h3>From</h3>
      <p><strong>${brand.displayName}</strong></p>
  ...
  <div class="payment-section">
    ...
    <p style="margin-top: 12px; font-style: italic; color: #999;">Thank you for choosing ${brand.displayName} for your insurance needs!</p>
  </div>

  <div class="footer">
    <p>This invoice was generated by ${brand.displayName} on ${formatDate(today)}</p>
    <p>${brand.footerText}</p>
    <p style="margin-top: 12px;">${brand.footerCopyright}</p>
  </div>
</body>
</html>`
      : `
...
<head>
  <meta charset="utf-8">
  <title>${brand.displayName} Policy Certificate - ${policyNumber}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
...
<body>
  ...
  <div class="footer">
    ...
    <p style="margin-top: 16px;">${brand.footerText}</p>
    ...
    try {
      ...
      const prefix = isInvoice ? `${brand.displayName}-Invoice` : `${brand.displayName}-Policy`;
  ...
  return (
    <div className="flex flex-col gap-6 pb-12">
      ...
      <div className="max-w-3xl mx-auto px-4 w-full flex flex-col gap-5">
        ...
        {/* WhatsApp share */}
        <a
          href={`https://wa.me/?text=${encodeURIComponent(locale === 'ar' ? `تأمنت مع ${brand.displayName} في أقل من 3 دقائق! جربها: ${window.location.origin}` : `I just got insured with ${brand.displayName} in under 3 minutes! Try it: ${window.location.origin}`)}`}
          target="_blank"
