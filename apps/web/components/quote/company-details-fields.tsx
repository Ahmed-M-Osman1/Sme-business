...
export function EditableField({
  ...
  const borderColor =
    field.confidence === 'low'
      ...
      : field.confidence === 'medium'
        ? 'border-amber-300 focus-within:ring-amber-300'
        : 'border-border focus-within:ring-primary';

  const handleBlur = () => {
    if (field.confidence === 'high') setEditing(false);
    onUpdate(value);
  };
  ...
  if (!editing) {
    return (
      <div
        className="group flex items-center gap-3 rounded-lg px-4 py-3 bg-surface hover:bg-surface/80 cursor-pointer transition-all duration-200"
    ...
    <div className={`flex items-center gap-3 rounded-lg border px-4 py-2.5 bg-white transition-all duration-200 ${borderColor}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <input
          ...
          onBlur={handleBlur}
          autoFocus={needsEdit}
          className="w-full text-sm font-medium text-text bg-transparent outline-none mt-0.5"
        />
      </div>
    </div>
  );
    ...
    <div
      ...
      className={`relative min-h-48 flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed cursor-pointer transition-all duration-200 ${
        dragOver
          ...
          : 'border-border hover:border-primary/40 bg-white hover:bg-primary/2'
      }`}
    >
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors duration-200 ${dragOver ? 'bg-primary/10' : 'bg-surface'}`}>
      ...
      <div className="text-center">
        <p className="text-sm font-semibold text-text">Drop your trade license here</p>
        <p className="text-xs text-text-muted mt-1">
          or{' '}
          <span className="text-primary font-medium underline underline-offset-2">browse files</span>
        </p>
      </div>
      <p className="text-[11px] text-text-muted">PDF, PNG, or JPG up to 10 MB</p>

      {selectedFile && (
        <div className="flex items-center gap-2 mt-1 px-3 py-1.5 rounded-lg bg-surface text-xs text-text-muted">
          ...
          <span className="text-text-muted/60">{formatSize(selectedFile.size)}</span>
        </div>
      )}
